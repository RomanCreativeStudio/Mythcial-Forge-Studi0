...
Per the Founder Directive amendment (`brand/Mythic_Forge_Studios_BRA-0001A_Brand_Bible_Amendment_Signature_Cinematic_Presentation.md`, Brand Bible Section 3 v1.1): these two assets are approved renderings of the same canonical BRA-0001 logo geometry, not a separate logo. Their Storage Location is **not a repository file** — this environment has no tool to export chat-submitted images to disk, so these entries document Founder-approved artwork by reference only. If a permanent repository copy is wanted, the source file needs to be added from outside this session (wherever the artwork was originally rendered) and this row updated with its real path.

| Asset ID | Asset Name | Version | Status | Creator | Approval Date | Storage Location | Notes |
|---|---|---|---|---|---|---|---|
| BRA-0001A-CINEMATIC-LOGO | Signature Cinematic Presentation — Standalone Logo Artwork | v1.0 | Approved | External (submitted by Founder), reviewed by Claude, approved by Founder | 2026-07-08 | *Not stored in repository — submitted as chat image, no file-export tool available in this environment* | Structural geometry (double horn, notch-origin fracture, shard-above-notch) verified consistent with the Locked BRA-0001 v1.0 master, based on repeated visual comparison during the v0.11/v0.12 tracing work. Pixel-exact geometric equivalence not verifiable without file access. |
| BRA-0001A-CINEMATIC-BANNER | Signature Cinematic Presentation — Official YouTube Banner | v1.0 | Approved | External (submitted by Founder), reviewed by Claude, approved by Founder | 2026-07-08 | *Not stored in repository — same capability gap as above* | Same structural geometry verification as BRA-0001A-CINEMATIC-LOGO. Three items remain unverified without file access: exact safe-area (1546×423) pixel placement, mobile-crop tagline legibility, and focal-contrast measurement against the background environment — disclosed as open QA items in the BRA-0001A record, not silently closed. This entry supersedes the informal "flagged, not certified" status from the immediately preceding review. |
