# THE FRACTURE PROTOCOL — PHASE 6B: CHARACTER ASSET PRODUCTION PACKAGE
## Character 3 of 4 — The Guardian

**Classification:** Internal — Production Document (**Compiled — Pending Founder Review**)
**Status:** Compiled 2026-07-15 per "Founder Directive — Phase 6B.0 Character Asset Production (v1.0)," Step 3 of 4 (Kael → Mira → **the Guardian** → Cassian). This document creates no story content, no canon expansion, and no screenplay revision — it is a production asset specification only.
**Absolute authority:** subordinate to Master System Prompt → Locked Canon (Phase 1, 1C Character Bible, 1C Faction Bible, Phase 2 series) → Production Bible → Approved Production Packages (Phase 4B) → Approved Screenplays (Phase 5B, Locked under Phase 5B.5) → Phase 6.0 → Phase 6A → Phase 6A.5, in that order.
**Working interpretation carried forward from Kael's and Mira's packages, applied identically here:** field-level missing specification is flagged and the package continues to build; only a structural stop condition halts production. The Guardian's Character Authority (identity, philosophy, role) is fully Locked and sufficient to build a package around, even where physical specification is entirely absent.
**A distinct structural note for this character, stated explicitly rather than assumed:** unlike Kael and Mira, the Guardian's non-humanoid, form-unspecified nature (Locked: "an ancient artificial intelligence," Phase 1A) means several of the directive's 10 sections may be genuinely **not applicable** rather than merely missing — most notably clothing/costume. This distinction (not-applicable vs. missing) is made explicit field-by-field below rather than conflated.

---

## 1. Character Authority

| Source | What It Governs | Authority Level |
|---|---|---|
| `bible/Fracture_Protocol_Phase1C_Character_Bible_v1.0.md`, "The Guardian" (lines 69–115) and "Relationship Dynamics — The Guardian & Kael" (lines 289–299) | Identity, philosophy, fears/limitations, relationship to Kael/Cassian, Internal Conflict, Character Arc, Why It Matters | Locked Canon |
| `bible/Fracture_Protocol_Phase1A_Production_Bible_v1.0.md`, Section 7 | Founding description: "ancient artificial intelligence," Core Principle ("Humanity cannot be led into a future it has not chosen"), origin left as open mystery | Locked Canon |
| `bible/Fracture_Protocol_Phase1B3_Production_Bible_v1.0.md`, Section 4 ("The Guardian Canon Lock") | Sworn protector of the Solar System; independent of all governments; provides guidance, does not rule | Locked Canon |
| `bible/Fracture_Protocol_Phase1C_Faction_Bible_v1.0.md` (Three-Way Conflict, Five/Six Interpretations of Protection) | The Guardian's "protection through wisdom" role among the four/six interpretations | Locked Canon |
| `bible/Fracture_Protocol_Phase5A2_ArcThree_Progression_Verification_Lock_v1.0.md` | Guardian mystery protection requirement — its origin must never be confirmed on-screen, in any episode, including its own arc-completing episode (Episode 32) and the finale (Episode 36) | Locked governance |
| 14 × Phase 5B screenplays with direct name+action-verb evidence (Episodes 1, 3, 4, 6, 7, 9, 10, 11, 13, 15, 16, 17, 18, 32); 10 × screenplays with a `GUARDIAN` speaker header (Episodes 4, 8, 18, 20, 24, 25, 32, 34, 35, 36) | Every scene, action line, and Character Performance Planning entry involving the Guardian — sole source for Expression/Pose/Animation/Voice *requirements* | Locked Production Canon (Phase 5B.5) |
| `mythic-forge-art-bible/` (all 11 files, checked) | **Zero Guardian-specific content found by name anywhere in the directory.** `lighting-system.md` was checked specifically (cited by name in Episodes 18, 24, 26, 32 as governing "the Guardian's established Season One visual language") and contains no Guardian-named content — only general System-Effect Lighting and Emotional Lighting Modes rules that are not Guardian-specific. **This is flagged as a genuine gap between what the screenplays assume exists and what the Art Bible actually specifies**, not resolved by treating the general lighting rules as if they were Guardian-specific. | N/A — confirmed silent; screenplay cross-reference flagged as unconfirmed |
| Phase 5B Episode 4, Production Notes (line 403) | Explicit Locked production note: "the Guardian's presence is established through performance and staging, not a visual effect, per the seed package's own explicit note" — directly relevant to Section 3/9 below | Locked Production Canon |
| `bible/Fracture_Protocol_Phase6A_Master_Production_Roadmap_v1.0.md` | Guardian's asset priority (Priority 2, 9 episodes) | Approved |
| `bible/Fracture_Protocol_Phase6A5_Asset_Production_Standards_v1.0.md` | Asset ID/Naming/Version/Lifecycle standards for Section 10 | Locked |
| `documentation/Asset_Registry.md` | Existing FPP-CHAR-004 backlog entry for the Guardian | Registry of record |

**Discrepancy noted, not silently resolved:** Phase 6A's registry lists 9 episodes for the Guardian. This session's `GUARDIAN` speaker-header grep found 10 files; a broader name+action-verb grep found 14. As with Mira, these different counts likely reflect different counting methods (speaking role vs. any mention) and are recorded rather than reconciled unilaterally.

---

## 2. Character Overview

**Identity.** The Guardian — an ancient artificial intelligence, the sworn protector of the Solar System, independent of all governments, commanded by no one. Its true origins (created by an ancient civilization, evolved independently, or something greater) remain, deliberately, one of the greatest mysteries of the universe — a Locked, permanently open thread that must never be resolved on-screen, including in its own arc-completing episode (32) and the series finale (36).

**Narrative purpose.** It exists to hold open the question the Ascendant Doctrine wants closed: whether guidance can coexist with restraint. Its Core Principle — "Humanity cannot be led into a future it has not chosen" — is a deliberate refusal to remove the thing that makes humanity's choices worth anything. It matters not because of its power or age, but because it is the one being in the story with every reason to give up on humanity and doesn't.

**Emotional role.** Its external goal is ensuring humanity survives without losing its ability to choose its own future; its private internal goal is understanding whether it truly belongs alongside humanity or will always remain an observer. Its hidden fear is not destruction but that its own restraint may have allowed preventable suffering — that Cassian's argument might be correct. Its heaviest burden is memory itself: it remembers every failed civilization, all at once, permanently.

**Thematic role.** "Protection through wisdom," one of the Faction Bible's interpretations of protection. Its arc moves it from a timeless, pattern-based observer to a partner walking beside humanity rather than standing watch over it — learning from Kael that humanity isn't a system to protect but people worth protecting.

No physical, costume, or purely-visual content appears anywhere in this section's source material.

---

## 3. Physical Production Profile

**Structurally different from Kael's and Mira's gaps — flagged as Not Applicable where the Guardian's Locked nature makes a field structurally inapplicable, and Missing where a field could in principle apply but has no Locked source.**

| Field | Status |
|---|---|
| Height | **Missing/Not Applicable — genuinely undetermined.** No Locked source states whether the Guardian has a fixed physical scale at all. |
| Proportions / body type | **Missing/Not Applicable — no Locked form exists.** The Guardian is an "ancient artificial intelligence" with no confirmed physical embodiment anywhere in Locked canon. |
| Silhouette | **Missing.** Even a non-humanoid entity typically requires a recognizable silhouette for character-recognition purposes in animation and marketing; none is specified. |
| Posture | **Not Applicable, pending form determination** — cannot be specified before a form (embodied, projected, ambient, or otherwise) is established. |
| Movement language | **Flagged, contradictory evidence.** Direct name+action-verb grep evidence exists across 14 screenplays ("Guardian sits," "stands," "speaks," "turns," etc. — verbs that imply a discrete, staged physical presence in at least some scenes). This sits alongside Episode 4's explicit Locked production note that "the Guardian's presence is established through performance and staging, not a visual effect" — which is consistent with a staged, performed presence, but still does not confirm a specific physical form, silhouette, or embodiment. **This tension — action-verb evidence implying physical staging, against zero Locked visual-form specification — is flagged explicitly rather than resolved by inventing a form.** |
| Facial characteristics | **Not Applicable, pending form determination.** |
| Hair | **Not Applicable, pending form determination.** |
| Eyes | **Not Applicable, pending form determination.** No eye-glow or Signal-linked system is established for the Guardian (that system is Locked as Kael-specific). |
| Skin tone / complexion | **Not Applicable, pending form determination.** |
| Clothing / equipment | **Not Applicable.** An ancient artificial intelligence with no confirmed physical embodiment has no basis for a clothing/equipment specification; this is a structural non-applicability, not a missing-data gap, and should not be conflated with Kael's or Mira's genuinely missing uniform/costume data. |

**The one confirmed production instruction (Locked, Episode 4, line 403 and line 186):** the Guardian's presence must be established through **performance and staging**, not a visual effect, and its presence is **not marked by any lighting change** relative to the surrounding scene. This is a Locked negative constraint — what NOT to do (no VFX reveal, no lighting shift) — not a positive form specification. It is recorded here as the closest thing to a "physical production" rule that exists for the Guardian, and should govern any future form design (i.e., whatever form is eventually chosen, it must still be renderable without a VFX-driven reveal or a lighting cue, per this Locked instruction).

**Rendering register (applies once/if a form is specified):** per `forbidden-elements.md`, the general "semi-realistic anime cinematic hybrid" register applies to all characters; no Guardian-specific exception or addition exists.

---

## 4. Costume Breakdown

**Not Applicable, structurally — not merely missing.** The Guardian has no confirmed physical embodiment in Locked canon, and therefore no basis exists for a costume breakdown of any kind. This determination is made explicitly rather than defaulting to "flag as missing" the way Kael's and Mira's genuinely absent-but-otherwise-humanoid costume data was flagged — the Guardian's case is categorically different and is recorded as such. Should a future Founder directive or Art Bible expansion establish a physical form for the Guardian, this section would need to be rebuilt from that new specification, not inferred from this one.

---

## 5. Expression Library

The 10 required expressions (fixed list, per directive). **Flagged as structurally uncertain**, pending the same form-determination gap as Section 3: an entity with no confirmed face has no confirmed basis for facial expressions in the conventional sense. Recorded below as required narrative *states* the Guardian must be able to convey — through voice, staging, and whatever eventual visual/performance solution is chosen — rather than as confirmed facial-expression targets.

| Required state | Narrative grounding |
|---|---|
| Neutral | Its baseline "calm and certain on the outside" register (Character Bible, Character Arc). |
| Focused | Evaluating civilization-scale patterns/futures (its established function). |
| Concerned | Its hidden fear that its restraint allowed preventable suffering. |
| Determined | Its Core Principle, held even under direct challenge (e.g., from Cassian). |
| Hopeful | Its choice to keep believing in humanity despite "every reason to give up." |
| Fear | Its hidden fear (Section 2) — "quietly exhausted underneath" its calm exterior. |
| Grief | Its heaviest burden — remembering every failed civilization, permanently. |
| Anger | Not evidenced in the Character Bible or screenplay grep sweep — flagged as narratively ungrounded for this character, consistent with its Locked "not anger" framing even toward Cassian ("respect, understanding, and profound regret — not anger"). |
| Compassion | Its relationship with Cassian ("brilliant mind carrying an unbearable responsibility," not an enemy). |
| Reflection | Its Internal Conflict — understanding whether it truly belongs alongside humanity. |

---

## 6. Pose Library

Cross-checked against direct screenplay evidence, with the same structural caveat as Sections 3 and 5: "pose" presumes a physical form that is not itself Locked.

| Pose | Required? | Evidence |
|---|---|---|
| Standing | Flagged | Action-verb evidence ("stands") found, but form-undetermined (Section 3). |
| Walking | Flagged | Not clearly evidenced as distinct from general "moves"/"steps" verbs; form-undetermined. |
| Running | **Not required** | No evidence found; inconsistent with the Guardian's established "calm and certain," non-combat narrative register. |
| Combat Ready | **Not required** | No Locked or screenplay evidence of the Guardian in a combat role; its Locked function is guidance/protection through wisdom, not direct combat. |
| Conversation | Yes | Speaker header confirmed in 10 episodes; dialogue-bearing role is Locked and central (its relationship with Kael and Cassian are both dialogue-driven). |
| Observation | Yes | Consistent with its Locked "observer" framing throughout the Character Bible (Character Arc: "having come to understand humanity mostly through pattern... at a distance"). |
| Rest | **Not required** | No evidence; an ancient AI with no confirmed physical form has no clear basis for a "rest" pose distinct from Neutral/Conversation staging. |
| Leadership | Flagged | Its guidance role is Locked, but "Leadership" as a staged pose (vs. Kael's command-briefing staging) has no direct screenplay evidence; flagged rather than assumed. |
| Silhouette | Yes | Required as a marketing/key-art pose per Phase 6A's asset planning convention — but see Section 3's flagged silhouette gap; this requirement cannot be fulfilled until a form exists. |

Of the nine fixed-list poses, only two (Conversation, Observation) are confirmed both by direct evidence and consistency with Locked characterization; the rest are flagged, excluded, or blocked on the same form-determination gap running through this entire package.

---

## 7. Animation Requirements

Requirements only, with the form-determination caveat applying throughout.

- **Idle:** flagged — cannot be specified without a form.
- **Walk cycle:** **not required** — no combat/urgency or general locomotion evidence found; consistent with an "ambient civilizational presence" framing (Episode 4, Story Purpose: "Establish the Guardian as ambient civilizational presence before any personal contact").
- **Run cycle:** **not required.**
- **Turn:** flagged — action-verb evidence exists ("turns") but form-undetermined.
- **Look/head-track:** flagged — presumes a face/head, which is not confirmed (Section 3).
- **Conversation animation:** required — dialogue-bearing role confirmed across 10 episodes; the specific mechanism (voice-only, staged presence, or something else) is exactly the open question Section 3 flags.
- **Combat animation:** **not required.**
- **Reaction animation:** required at the *voice/performance* level (its Internal Conflict must be conveyable) even though the *physical* animation question remains open.
- **Facial animation:** flagged — presumes a face; not confirmed to apply.

**Governing production instruction repeated from Section 3:** whatever animation solution is eventually chosen must satisfy the Locked constraint that the Guardian's presence is established through performance and staging, not a visual effect, and is not marked by a lighting change.

---

## 8. Voice Requirements

Tone, pace, energy, and emotional range only. **No casting or actor suggestion is made here.** This is the Guardian's most concretely specifiable section, since its Locked characterization is philosophical/verbal rather than physical.

- **Tone:** calm, measured, carrying centuries of patience without sounding detached — "calm and certain on the outside, quietly exhausted underneath" (Character Bible, Character Arc). Episode 8's Locked production note is directly relevant: "the Guardian's slightly longer pause before speaking is the only permitted signal of its underlying weight — nothing more overt, nothing named or commented on." Pacing restraint is itself a Locked characterization tool for this character.
- **Pace:** deliberate, unhurried — consistent with the "slightly longer pause" rule above; never rushed, even under challenge (e.g., scenes with Cassian).
- **Energy range:** must span serene philosophical authority (Core Principle statements) to the quiet, privately-carried weight of its hidden fear and its memory-burden — without ever becoming performed or theatrical, per its Locked "nothing more overt" constraint.
- **Emotional range:** respect and regret (never anger) toward Cassian; partnership warmth (not superiority) toward Kael; must be able to carry genuine uncertainty about its own belonging without undermining its calm, authoritative baseline register.

---

## 9. Asset Dependencies

| Dependency category | Entries |
|---|---|
| Costumes | **Not Applicable** (Section 4). |
| Equipment | **Not Applicable** — no confirmed physical form. |
| Weapons | None — consistent with Phase 6A's confirmed zero-new-weapons finding and the Guardian's non-combat role. |
| Props | None found. |
| Environments | Depends on Phase 6A's Environment backlog; the Guardian's scenes are consistently staged as "ambient civilizational presence" (Episode 4) within existing environments rather than a dedicated Guardian-specific location. |
| Effects | **Explicitly excluded by Locked production note** (Episode 4, line 403: "None required — the Guardian's presence is established through performance and staging, not a visual effect"). This is a confirmed *absence* of a VFX dependency, not a gap. |
| Storyboards | Depends on Phase 6A's Storyboard Plan; not yet asset-produced. |
| Animation | Depends on Section 7 above, most of which is blocked on form-determination; not yet asset-produced. |
| Voice | Depends on Section 8 above — the most production-ready dependency for this character; voice cast confirmed not-yet-existing (Phase 5C Recommendation R-2). |
| Marketing | Silhouette pose (Section 6) flagged as required but currently unfulfillable pending form-determination (Section 3). |

---

## 10. Asset Registry Entry

**Same ID-format inconsistency as Kael's and Mira's packages, applied here:** Phase 6A uses `FPP-CHAR-004`; Phase 6A.5's Asset ID Standard specifies `CHR-0004`. Both recorded, not unilaterally reconciled.

**Episode-count discrepancy also flagged:** Phase 6A's registry lists 9 episodes; this pass found 10 files with a `GUARDIAN` speaker header and 14 with direct name+action-verb evidence. Recorded as-is.

**The central open item for this character, restated at the registry level:** unlike Kael and Mira (where the gap is "missing physical detail for an otherwise-humanoid character"), the Guardian's asset production cannot proceed to concept art or turnaround at all until a Founder decision establishes whether — and how — the Guardian has a physical form, consistent with the Locked "performance and staging, not a visual effect" constraint. This is the single largest open production question surfaced across all three character packages built so far.

| Field | Value |
|---|---|
| Legacy Asset ID (Phase 6A) | FPP-CHAR-004 |
| Standard-format ID (Phase 6A.5) | CHR-0004 *(proposed, pending Founder reconciliation of the two ID schemes)* |
| Name | The Guardian |
| Category | Character |
| Naming convention (Phase 6A.5) | `CHR_0004_Guardian_v001` |
| Production status | Requested → **Blocked on form-determination** *(distinct from Kael's and Mira's "In Progress" status — this character cannot proceed to visual asset production until the Founder resolves whether/how it has a physical form)* |
| Version | v1.0 (this package) |
| Priority | 2 (Phase 6A Asset Priority Matrix) |
| Episodes used | Phase 6A registry: 9. This pass: 10 (speaker header), 14 (name+action-verb evidence) — discrepancy flagged above, not resolved. |
| Dependencies | See Section 9 |
| Creator | Compiled per Phase 6B.0 directive execution |
| Creation date | 2026-07-15 |
| Approval date | *(pending — set upon Founder Approval below)* |

---

## Founder Review & Alignment Audit

| Check | Result |
|---|---|
| Repository integrity | PASS — all cited sources located and read directly. |
| Authority integrity | PASS — authority chain matches Phase 6B.0 Section 2 and prior packages' precedent. |
| Character integrity | PASS — Section 2 sourced directly from Character Bible with no invention; the Guardian mystery-protection rule (Phase 5A.2) is explicitly respected — no origin theory is asserted as fact anywhere in this document. |
| Screenplay consistency | PASS, with a flagged internal tension — direct action-verb evidence across 14 screenplays implies staged physical presence, while zero Locked visual-form specification exists; this tension is stated explicitly in Section 3 rather than resolved by invention in either direction. |
| Visual consistency | **N/A — no visual specification exists for the Guardian anywhere in the Art Bible.** The screenplays' own repeated cross-reference to "the Guardian's established Season One visual language, per `lighting-system.md`" was checked directly and found **not actually present** in that file under any Guardian-specific heading — flagged as a discrepancy between what four separate screenplays assume exists and what the cited source actually contains, not silently patched over. |
| Dialogue consistency | N/A — no dialogue authored here. |
| Asset consistency | PASS — zero new props/weapons/VFX invented; the "no visual effect" constraint (Episode 4) is honored, not overridden. |
| Dependency consistency | PASS — Section 9 correctly marks Costumes/Equipment as Not Applicable rather than Missing, and Effects as a confirmed Locked absence rather than a gap. |
| Registry consistency | **FLAGGED, not blocking** — FPP-CHAR-004 vs. CHR-0004 ID format, and 9 vs. 10 vs. 14 episode-count discrepancy, all recorded rather than resolved unilaterally. |
| Production readiness | **BLOCKED beyond Sections 1, 2, 5 (partial), 8.** Sections 3, 4, 6, 7, 9 (partial), and 10 (production status) are structurally gated on a single open question — whether/how the Guardian has a physical form — which this document identifies but does not and cannot resolve on its own authority. |

**Determination: PASS, with the most significant open item of any character processed so far under Phase 6B.0.** No canon contradiction, repository corruption, governance conflict, or asset dependency conflict was found — but the form-determination question is flagged as functionally blocking for visual asset production (concept art, turnaround, animation) even though it does not meet this document's own definition of a package-level STOP CONDITION (it does not contradict canon; it exposes an genuine absence of canon on this specific point). Consistent with the working interpretation applied to Kael and Mira, this is treated as the largest field-level flag yet, not a halt — the Founder may instead choose to treat this as requiring a new directive before any further Guardian asset work proceeds; that determination is the Founder's to make.

---

## Founder Approval

*(Pending — awaiting explicit Founder Approval before this section is completed and the document promoted from "Compiled — Pending Founder Review" to "Approved.")*

---

### Changelog
`[v1.0 — 2026-07-15] Compiled per "Founder Directive — Phase 6B.0 Character Asset Production (v1.0)," Character 3 of 4 (the Guardian). All 10 required sections built from direct repository citation only, with the Guardian mystery-protection rule (Phase 5A.2) fully respected — no origin theory asserted as fact. The largest open production question surfaced across all Phase 6B.0 characters so far: whether/how the Guardian has any physical form at all, given zero Locked visual specification exists anywhere (Art Bible, World Bible, Faction Bible, Phase 1B.3) against contradicting evidence of staged physical presence across 14 screenplays' action lines. Four screenplays (Episodes 18, 24, 26, 32) cross-reference "the Guardian's established Season One visual language, per lighting-system.md" — checked directly and found not actually present in that file, flagged as a genuine discrepancy rather than silently patched. Sections 3, 4, 6, 7, and 9 explicitly distinguish Not Applicable (structurally excluded by the Guardian's non-physical Locked nature) from Missing (could apply but lacks a source) rather than conflating the two, unlike Kael's and Mira's packages where nearly every gap was a true Missing. Registry Section 10 marks production status as "Blocked on form-determination," distinct from Kael's and Mira's "In Progress." Status: "Compiled — Pending Founder Review."`
